...
export function createTokenBudgetManager(budget: contextBudget): TokenBudgetManager {
  ...
  function validateBudget(components: Record<string, string>): ValidationResult {
    const violations = [];
    ...
    for (const [componentName, text] of Object.entries(components)) {
      ...
      if (used > limit) {
        violations.push({ component: componentName, used, limit, over: used - limit });
      }
    }
  ...
  function getMetrics(components: Record<string, string>): BudgetMetrics {
    ...
    const overallUtilization =
      totalBudget > 0 ? Number(((totalUsed / totalBudget) * 100).toFixed(2)) : 0;

    return { utilization, totalUsed, totalBudget, overallUtilization };
  }

  return { getTokenCount, validateBudget, truncateToFit, getMetrics };
}
